    ...
    public Flux<BookDto> findAll() {
        return template.getDatabaseClient().inConnectionMany(connection ->
                Flux.from(connection.createStatement(FIND_ALL)
                                .execute())
                        .flatMap(result -> result.map(this::mapToDto)));
    }

    private BookDto mapToDto(Readable selectedRecord) {
        ...
        AuthorDto author = getAuthorDto(authorJson);
        ...
        List<GenreDto> genres = getGenreDto(genresJson);

        return new BookDto(id, title, author, genres);
    }

    private AuthorDto getAuthorDto(String authorJson) {
        try {
            JsonNode node = objectMapper.readTree(authorJson);
            return new AuthorDto(node.get("id").asLong(),node.get("name").asText());
        } catch (JsonProcessingException e) {
            throw new IllegalArgumentException("Author JSON parsing error: " + authorJson, e);
        }
    }

    private List<GenreDto> getGenreDto(String genreJson) {
        try {
            List<GenreDto> genres = objectMapper.readValue(genreJson, new TypeReference<>() {
            });
            return new ArrayList<>(genres);
        } catch (JsonProcessingException e) {
            throw new IllegalArgumentException("Genre JSON parsing error: " + genreJson, e);
        }
    }
}
